		...
		for degree in _k:
			...
			if (degree == 0):
				...
				if t[i] == t[i + degree]:
					...
					a[i][degree] = lambda x, i=i, degree=degree: ((x - t[i]) / (t[i + degree] - t[i]))
				b[i].append(
					lambda x, i=i, degree=degree: a[i][degree](x) * b[i][degree - 1](x) + (1 - a[i + 1][degree](x)) * b[i + 1][degree - 1](x))
	return b  # set b
...
for power in range(4):
	...
	step_size = 1.0 / (m - degree * 2)  # t between clamped ends will be evenly spaced (not a necessary condition, however)
	t = [i * step_size for i in range(-degree, 0)] + [i * step_size for i in range(0, m - (degree * 2) + 1)] + [1+ i * step_size for i in range(1, degree+1)]
	b = deboor(degree, t)
